...
from ghidra.app.util.ByteCopier import ByteViewerTransferable
...
def read_bytes(address, nbytes):
    ...
    if currentProgram.getMemory().getBytes(address, signed_chars) != nbytes:
...
class KeilDecompress(object):
    ...
    def run(self):
        ...
        if currentSelection is None or currentSelection.isEmpty():
            yikes("Please select the range of memory you believe is the compressed init data.")

        range = currentSelection.getFirstRange()
        packed_data = read_bytes(range.getMinAddress(), range.getLength())
        unpacked_size = askInt("Keil Unpacker", "Please enter the unpacked size")
        ...
        data = ByteViewerTransferable(hexlify(bytearray(unpacked_data)))
        clipboard.setContents(data, None)
        print("Decompression successful, decompressed data copied to clipboard!")
